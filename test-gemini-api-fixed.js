// Updated test script for Gemini API integration
...
async function testGeminiAPIDirectly() {
  ...
  try {
    ...
    // Find the correct Gemini model name from the available models
    const geminiModels = modelsResponse.data.models.filter(model => 
      model.name.includes('gemini') && model.supportedGenerationMethods?.includes('generateContent')
    );
    
    if (geminiModels.length === 0) {
      throw new Error('No suitable Gemini models found for content generation');
    }
    
    // Use the first available Gemini model that supports content generation
