...
git config --global user.email "manus.twin@example.com"
git config --global user.name "Manus Twin"
...
# Get the GitHub username directly
USERNAME=$(curl -s -H "Authorization: token $GITHUB_TOKEN" https://api.github.com/user | grep -o '"login":"[^"]*"' | sed 's/"login":"//;s/"//')
if [ -z "$USERNAME" ]; then
    echo -e "${RED}Could not determine GitHub username. Please check your token.${NC}"
...
echo -e "${GREEN}Repository URL: https://github.com/$USERNAME/$REPO_NAME${NC}"
